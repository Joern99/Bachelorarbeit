...
\newlength{\origcftsecnumwidth}
\newlength{\origcftsubsecnumwidth}
\setlength{\origcftsecnumwidth}{\cftsecnumwidth}
\setlength{\origcftsubsecnumwidth}{\cftsubsecnumwidth}
...
% Zugriffdatum ("besucht am") im Literaturverzeichnis nicht ausgeben
...
% Vortexte mit römischer Seitennummerierung
\pagenumbering{Roman}

\section*{Abstract / Zusammenfassung}
\phantomsection
\addcontentsline{toc}{section}{Abstract / Zusammenfassung}

Large Language Models (LLMs) werden zunehmend in wissensintensiven Kontexten eingesetzt, wobei die verlässliche Bewertung ihrer Antworten an Bedeutung gewinnt. Plausibilität stellt dabei eine zentrale, aber schwer zu messende Qualitätsdimension dar, da LLMs häufig sprachlich überzeugende, jedoch nicht zwingend inhaltlich belastbare Antworten erzeugen. Diese Arbeit untersucht die Frage, welche quantitativen und qualitativen Diskrepanzen zwischen automatischen Evaluationsmetriken und menschlichen Plausibilitätsbewertungen bestehen.

Zur Beantwortung dieser Forschungsfrage wird Plausibilität als mehrdimensionales Konstrukt operationalisiert und in die Dimensionen Kontextverständnis, Kohärenz und Angemessenheit unterteilt. Die theoretische Fundierung erfolgt durch das Plausibility Analysis Model (PAM) und die Knowledge-Fitting Theory nach Connell und Keane. Acht deutschsprachige Frage-Antwort-Paare aus dem German-RAG-LLM-HARD-BENCHMARK wurden von GPT-OSS-120B beantwortet und anschließend sowohl durch die LLM-as-a-Judge-Metrik G-Eval mit GPT-4.1 als auch durch menschliche Rater entlang der definierten Dimensionen bewertet.

Die automatische Metrik zeigt hohe Intra-Rater-Reliabilität (Kendalls $W = 0.810$), weist jedoch extreme Deckeneffekte bei Angemessenheit (Standardabweichung = 0.13) und Kohärenz (Standardabweichung = 0.01) auf. Die menschlichen Bewertungen erreichen durchgängig niedrige Inter-Rater-Reliabilität über alle Dimensionen hinweg (Kendalls $W$ zwischen 0.11 und 0.26), was nahe am Zufallsniveau liegt. Der Vergleich beider Ansätze zeigt substanzielle Divergenzen: Die dimensionsspezifischen Korrelationen gemessen mit Kendalls $\tau$ variieren zwischen -0.214 (Kohärenz) und 0.509 (Angemessenheit). Bemerkenswert ist, dass die holistische Plausibilitätsbewertung nach Zhang mit Kendalls $\tau = 0.786$ deutlich höhere Übereinstimmung zeigt als die operationalisierten Einzeldimensionen.

Die Arbeit zeigt, dass LLM-as-a-Judge-Metriken prinzipiell zur Plausibilitätsbewertung geeignet sein können, jedoch unter den gewählten Bedingungen systematische Limitationen aufweisen. Die extreme Varianzlosigkeit bei zwei Dimensionen sowie die systematische Aufwärtsverzerrung um über einen Skalenpunkt deuten auf Prompt-Sensitivität und Modell-Bias hin. Gleichzeitig dokumentiert die niedrige Inter-Rater-Reliabilität der menschlichen Referenzmessung über alle Dimensionen ein fundamentales Validierungsproblem, da sich konstruktbedingte Subjektivität und methodisches Rauschen nicht sicher trennen lassen. Für zukünftige Forschung werden theoriegeleitete Weiterentwicklungen der Messdimensionen, systematische Prompt-Variation und die Integration von Log-Wahrscheinlichkeiten zur Reduktion von Deckeneffekten vorgeschlagen.
...
Diese Arbeit wurde unter Nutzung KI-gestützter Werkzeuge erstellt. Die Verantwortung für Inhalt, Auswahl der Quellen, Argumentation sowie alle finalen Formulierungen liegt beim Autor.
...
\acro{pam}[PAM]{Plausibility Analysis Model}
\end{acronym}
...
Damit ergibt sich ein zentrales methodisches Problem: Plausibilität ist für Nutzerinnen und Nutzer oft unmittelbar wahrnehmbar, zugleich aber schwer zuverlässig zu messen, insbesondere wenn sprachliche Überzeugungskraft und faktische Korrektheit auseinanderfallen. Für Forschung und Praxis stellt sich daher die Frage, inwieweit automatische Evaluationsmetriken Plausibilität so erfassen können, dass ihre Ergebnisse mit menschlichen Urteilen vergleichbar sind.

Aus der Problemstellung ergibt sich die folgende Forschungsfrage:

\enquote{Welche quantitativen und qualitativen Diskrepanzen bestehen zwischen automatischen Evaluationsmetriken und der menschlichen Plausibilitätsbewertung in den Dimensionen Kontextverständnis, Kohärenz und Angemessenheit?}

Ziel der Arbeit ist es, die Übereinstimmung beider Bewertungsansätze quantitativ zu bestimmen und qualitative Abweichungsmuster pro Dimension zu identifizieren. Daraus wird abgeleitet, für welche Aspekte der Plausibilitätsbewertung automatische Verfahren zuverlässig einsetzbar sind und in welchen Bereichen menschliche Bewertung weiterhin notwendig bleibt.

Die Arbeit versteht Plausibilität als mehrdimensionales Konstrukt (vgl. \parencite{klein_plausibility_2023}; \textcite{connell_model_2006}) und operationalisiert sie in den drei Dimensionen Kontextverständnis, Kohärenz und Angemessenheit. Kontextverständnis beschreibt, inwieweit eine Antwort die gegebenen Informationen korrekt aufgreift und integriert. Kohärenz erfasst die innere logische Stimmigkeit und Nachvollziehbarkeit der Argumentation. Angemessenheit bewertet, ob Umfang, Detailtiefe und Schwerpunktsetzung der Antwort zur jeweiligen Fragestellung passen.

Zur Beantwortung der Forschungsfrage werden zwei Bewertungsansätze parallel eingesetzt. Erstens werden menschliche Plausibilitätsurteile auf einer mehrstufigen Skala erhoben (vgl. \parencite{moosbrugger_testtheorie_2020}). Zweitens werden dieselben Antworten mittels G-Eval automatisiert entlang derselben Dimensionen bewertet. Die resultierenden Daten werden quantitativ ausgewertet und durch qualitative Analysen ergänzt.

Der Aufbau der Arbeit ist wie folgt: Abschnitt~2 legt die theoretischen Grundlagen zu \ac{llm}s, Plausibilität und Evaluationsmetriken dar. Abschnitt~3 beschreibt das Untersuchungsdesign, die Datengrundlage sowie die angewandten Auswertungsmethoden. Abschnitt~4 präsentiert die Ergebnisse, die in Abschnitt~5 eingeordnet und diskutiert werden. Abschnitt~6 schließt mit einem Ausblick. Ergänzende Materialien und zusätzliche Auswertungen sind im Anhang dokumentiert.
...
Ihr Funktionsprinzip beruht auf der statistischen Modellierung von Textsequenzen. Das Modell approximiert dabei die Wahrscheinlichkeitsverteilung des nächsten Tokens unter der Bedingung einer gegebenen Eingabesequenz (vgl. \parencite{brown_language_2020}). Das Modell wiederholt diese 
...
Als formaler theoretischer Rahmen für die Erklärung von Plausibilitätsurteilen dient das \ac{pam} nach \textcite{connell_model_2006}. Dieses Modell beschreibt Plausibilität nicht als intuitiven Prozess, sondern spezifiziert die kognitiven Mechanismen, die der menschlichen Bewertung von Szenarien zugrunde liegen. Es basiert auf empirischen Befunden, die eine fundamentale Unterscheidung zwischen sprachlicher und inhaltlicher Kohärenz treffen (vgl. \parencite{connell_what_2004}).
...
Zwar existieren Ansätze zur Erfassung von Plausibilität, diese fokussieren jedoch meist auf eine globale Gesamteinschätzung. Ein prominentes Beispiel ist die \ac{oci}-Skala nach \textcite{zhang_ordinal_2017}, die auch in aktuellen LLM-as-a-Judge-Studien wie bei \textcite{palta_everything_2025} Anwendung findet.
 Um jedoch der in Abschnitt~\ref{subsec:pam} beschriebenen Komplexität des Plausibilitätsbegriffs gerecht zu werden, bedarf es einer differenzierten Betrachtung, die über eine einzelne Messgröße hinausgeht und die zugrunde liegenden kognitiven Dimensionen explizit erfasst.

Diese Arbeit entwickelt daher ein mehrdimensionales Modell, das systematisch aus etablierten kognitiven Theorien abgeleitet ist. Die \ac{oci}-Skala dient als etablierter Validierungs- und Vergleichsparameter zur numerischen Grundlage der mehrdimensionalen Bewertung.
Als theoretischer Rahmen dienen das \ac{pam} und die \textit{Knowledge-Fitting Theory} (siehe Abschnitt~\ref{subsec:pam}).

Für die Bewertung natürlichsprachlicher Antworten von Large Language Models ist eine direkte Übernahme der \ac{pam}-Parameter ($P, L, N$) jedoch nicht geeignet. Das Modell wurde für die Simulation kognitiver Prozesse entwickelt und nicht für die Analyse textueller Produkte. Da bei natürlichsprachlichen Texten von \ac{llm}s keine formale Wissensrepräsentation vorliegt, ist die direkte Berechnung von \ac{pam}-Parametern aus Textdaten methodisch nicht umsetzbar.
...
Diese Dimension adaptiert den Faktor der Vermeidung von Spekulation (\textit{Conjecture}, siehe Abschnitt~\ref{subsec:pam}). Im \ac{pam} sinkt Plausibilität, wenn hypothetische Zusatzannahmen nötig sind.
...
Annotation mit mehreren Bewertenden pro Item durch.
...
\begin{table}[H]
\centering
\caption{Kendall-Tau-Korrelation verschiedener Evaluationsmetriken mit menschlichen Urteilen auf dem SummEval-Benchmark \parencite[S.~4]{liu_g-eval_2023}}
...
Kendall \(W\) beruht auf dem Prinzip, dass bei vollständiger Übereinstimmung 
...
Für die Dimension der Gesamtplausibilität wird die \ac{oci} Skala verwendet. Da diese Skala bereits in der Praxis etabliert ist (siehe Abschnitt~\ref{subsec:oci-skala}) und im \ac{llm}-Prompt explizit definiert wurde (siehe Abschnitt~\ref{subsec:implementierung-geval}), wird sie hier identisch übernommen. Dies stellt sicher, dass die ganzheitliche menschliche Einschätzung auf demselben theoretischen Konstrukt fußt wie die maschinelle Bewertung.
...
\subsection{Komparative Analyse: Mensch versus Maschine}
...
\renewcommand{\theHsubsection}{app.\arabic{section}.\arabic{subsection}}

% Inhaltsverzeichnis: mehr Platz nur für Anhang-Einträge
\addtocontents{toc}{\protect\setlength{\protect\cftsecnumwidth}{6em}}
\addtocontents{toc}{\protect\setlength{\protect\cftsubsecnumwidth}{7em}}
% Inhaltsverzeichnis im Anhang auf Section-Ebene halten (keine 1.1/1.2/… Einträge)
\addtocontents{toc}{\protect\setcounter{tocdepth}{1}}

\phantomsection
\section{Evaluationsmaterial}
\label{app:anhang}
\label{app:material}
\subsection{Kontext}
\label{app:Kontext}
...
\clearpage
% Kein separates Überschriftsblatt: \includepdf startet immer auf einer neuen Seite.
\refstepcounter{section}
\phantomsection
\addcontentsline{toc}{section}{\protect\numberline{\thesection}Original-Fragebogen}
\label{app:fragebogen}
% Überschrift auf der ersten PDF-Seite ausgeben (ohne eigene "Leerseite")
\includepdf[pages=1, scale=0.9,
    pagecommand={\thispagestyle{plain}\noindent\normalfont\fontsize{11pt}{16.5pt}\bfseries \thesection\quad Original-Fragebogen\par\vspace{6pt}}]{Anhang/Fragebogen.pdf}
\includepdf[pages=2-, scale=0.9, pagecommand={\thispagestyle{plain}}]{Anhang/Fragebogen.pdf}
...
% Inhaltsverzeichnis: Standardbreiten/Tiefe vor dem Literaturverzeichnis wiederherstellen
\addtocontents{toc}{\protect\setlength{\protect\cftsecnumwidth}{\origcftsecnumwidth}}
\addtocontents{toc}{\protect\setlength{\protect\cftsubsecnumwidth}{\origcftsubsecnumwidth}}
